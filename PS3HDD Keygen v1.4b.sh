#!/bin/bash
clear
echo -e "\e[1m\e[92mHDD and VFLASH keys generator v1.4\e[0m"
...
case $model in
	...
	echo "359F59BB8C256B91093A92007203ABB33BADF5AC09A0DC005859D6F159C4F54F929214D8FCCB4CE7099ACEBDFC6612B9" | xxd -r -p > eid_root_key.bin
	...
	echo "C19C7F987EDB6E244B07BEDEFA1E6CC9F08524D98C05654CC742141E01F823E1" | xxd -r -p > encdec_tweak_seed.bin
      # echo "5F20A21ED12FF6425B62FDE0D1881C845F20A21ED12FF6425B62FDE0D1881C84" | xxd -r -p > hdd_key.bin
    ...
    x)
      # rm *.bin
      # rm eid_root_key.bin
	rm ata_data_seed.bin
	rm ata_tweak_seed.bin
	rm ata_data_and_tweak_seed.bin
	rm encdec_data_seed.bin
	rm encdec_tweak_seed.bin
	rm ata_data_key.bin
	rm ata_tweak_key.bin
	rm ata_data_and_tweak_key.bin
	rm encdec_data_key.bin
	rm encdec_tweak_key.bin
	rm hdd_key.bin
	rm flash_key.bin
	rm vflash_key.bin
	break;;
